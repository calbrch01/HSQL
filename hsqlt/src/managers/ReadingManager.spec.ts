...
describe('Extension tests', function () {
    ...
    //         const ft = FileHandler.getFileType(fn);
    //         assert.strictEqual(
    //             ft,
    //             answer,
    ...
    describe('Extension conversion', function () {
        ...
        it('ENUM based', async () => {
            assert.strictEqual(FileHandler.changeExtension(q1, FileType.ECL), q1);
            assert.strictEqual(FileHandler.changeExtension(q2, FileType.ECL), path.join('foo', 'bar.ecl'));
            assert.strictEqual(FileHandler.changeExtension(q3, FileType.ECL), 'foobar.temp.ecl');
        });
    });
    // Phased out as its blocked by the compiler
    // it('should error out selecting OTHER extension', async () => {
    //     const feh = new ReadingManager(ErrorManager.pedantic);
    //     try {
    //         FileHandler.changeExtension('./hello.ecl', FileType.OTHER);
